...
// Currently unported - kept for reference. Can be ported to EditorWindow but it's not trivial because
// of "SerializableType" and it being very coupled to the previous version that was inside the giant utility library.
// -
// Also, using roslyn custom codegen with a smaller c# script that just takes a .sln 
// makes more sense rather than using this window. (Microsoft.CodeAnalysis.*)
namespace BX.Tweening
{
    ...
    public class BXSTweenExtensionGeneratorTask : EditorTask
    {
        ...
        public string fileNamespace = "BX.Tweening";
        /// <summary>
        ...
        public override bool GetWarning()
        {
            ...
            if (File.Exists(GenerateFileAbsolutePath))
            {
                ...
                while (File.Exists(backupFileName))
                {
                    ...
                    {
                        throw new Exception($"[BXSTweenExtensionGeneratorTask::GetWarning] I have no idea how you managed to cram {ulong.MaxValue} amount of files into your file system, but congratulations."); // pretty plausible actually
                    }
                }
        ...
        public override void Run()
        {
            ...
            // constant using definitions (only add these if the root namespace isn't BX.Tweening)
            bool isBaseTweeningNamespace = fileNamespace.StartsWith("BX.Tweening");
            if (!isBaseTweeningNamespace)
            {
                currentNamespaceUsings.Add("BX.Tweening");
            }
            if (!isBaseTweeningNamespace || !fileNamespace.StartsWith("BX.Tweening"))
            {
                currentNamespaceUsings.Add("BX.Tweening");
            }
            ...
            foreach (ExtensionClassTemplate template in extensionPairs)
            {
                ...
                foreach (ExtensionMethodTemplate method in template.extensionMethods)
                {
                    ...
                    // (just don't shift+f12 StringBuilder.Append)
